...
| Model Name | BF16 | W8A8 INT8 | W4A8 dynamic INT4 |
|:---:|:---:|:---:|:---:|
| Llama-3.2-1B | [meta-llama/Llama-3.2-1B](https://huggingface.co/meta-llama/Llama-3.2-1B) | - | - |
| Qwen2.5-1.5B | [Qwen/Qwen2.5-1.5B-Instruct](https://huggingface.co/Qwen/Qwen2.5-1.5B-Instruct) | [RedHatAI/Qwen2.5-1.5B-quantized.w8a8](https://huggingface.co/RedHatAI/Qwen2.5-1.5B-quantized.w8a8) | [RedHatAI/Qwen2.5-1.5B-quantized.w4a16](https://huggingface.co/RedHatAI/Qwen2.5-1.5B-quantized.w4a16) |
...
The RVV backend supports W8A8 INT8 weight quantization and W4A8 dynamic INT4
execution from compressed-tensors INT4 checkpoints:

| Mode | Extra flags | Weight memory |
|------|-------------|---------------|
| BF16 (default) | *(none)* | baseline |
| W8A8 INT8 | `--quantization w8a8_int8` | −50% |
| W4A8 dynamic INT4 | `--quantization compressed-tensors` | −75% |
...
2. **W4A8 dynamic INT4:** Use a compressed-tensors INT4 checkpoint such as
   `RedHatAI/Qwen2.5-1.5B-quantized.w4a16`. The checkpoint stores W4A16-format
   weights, but the RVV backend packs them at load time and runs the W4A8 dynamic
   INT4 kernel at runtime.

For example, to use the RVV W4A8 dynamic INT4 path:

```bash
python3 -m sglang.launch_server \
    --model-path RedHatAI/Qwen2.5-1.5B-quantized.w4a16 \
    --attention-backend rvv \
    --device cpu \
    --dtype bfloat16 \
    --trust-remote-code \
...
On RVV, the attention backend also supports INT8 KV cache storage. This reduces
KV cache memory and can be combined with BF16, W8A8 INT8, or W4A8 dynamic INT4
weights:
...
To combine W8A8 weights with INT8 KV cache, use both flags:
...
To combine W4A8 dynamic INT4 weights with INT8 KV cache, use:
...
| `--quantization compressed-tensors` | Use compressed-tensors INT4 weights; on RVV this enables the W4A8 dynamic INT4 kernel |
| `--kv-cache-dtype int8` | Store KV cache in INT8 format using the RVV CPU attention backend |
