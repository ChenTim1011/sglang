...
def use_rvv_lm_head_backend(lm_head) -> bool:
    ...
    This is an eligibility check, not a guarantee that the module was already
    prepacked at load time. Unlike ordinary CPU linear weights, lm_head may
    need lazy packing to preserve tied embeddings and to tolerate post-load
    weight updates.
    """
    if not hasattr(lm_head, "weight"):
        return False
