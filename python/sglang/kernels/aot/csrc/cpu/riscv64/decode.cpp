...
inline float load_valid_int8_kv_scale(
  ...
  TORCH_CHECK(
      ...
      "decode_attention_int8_cpu: invalid ",
      scale_name,
      ...
      for (int64_t n = kv_start; n < kv_end; n += TARGET_BLOCK_N) {
        ...
        if (k_scale_buf) {
          ...
          for (int64_t j = 0; j < n_size; ++j) {
            k_scales_block[j] = load_valid_int8_kv_scale(k_scale_buf, cur_indices[j], num_heads_kv, head_kv_id, "K");
          }
        }
        ...
        if (v_scale_buf) {
          ...
          for (int64_t j = 0; j < n_size; ++j) {
            v_scales_block[j] = load_valid_int8_kv_scale(v_scale_buf, cur_indices[j], num_heads_kv, head_kv_id, "V");
          }
        }
