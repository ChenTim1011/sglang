...
def run_single_backend(backend_name, config, num_iterations=20, warmup=5):
    ...
    # RVV path: _rvv_process_weight_after_loading always converts bias to FP32
    bias_fp32 = torch.randn(N, dtype=torch.float32)
    # torch_native path: bias stays in BF16 (same dtype as model weights in E2E)
...
def print_result(result: BenchmarkResult):
    c = result.config
    params = f"[{c.M:4}x{c.K:4}] @ [{c.N:4}x{c.K:4}] BF16"
    print_benchmark_result(
        c.description,
        params,
        result.rvv_ms,
...
def main():
    parser = argparse.ArgumentParser(
        description="Benchmark RVV vs torch_native BF16 GEMM (E2E path)"
    )
    ...
    print("=" * 60)
    print("RVV BF16 GEMM Benchmark (E2E path — bfloat16)")
    print("=" * 60)
    print(f"Platform: {platform.machine()}")
    print(f"CI Mode: {IS_CI}")
    print("RVV:   weight_packed_linear(x_bf16, packed_w, bias_fp32)")
    print("Torch: F.linear(x_bf16, w_bf16, bias_fp32)  [torch_native E2E path]")
    print("-" * 60)
