...
#pragma omp parallel
  {
    alignas(64) float scores_block[32 * 64];
    alignas(64) scalar_t k_trans_buf[MAX_HEAD_SIZE * 64];
    alignas(64) scalar_t v_buf[64 * MAX_HEAD_SIZE];
        ...
        for (int m_start = 0; m_start < extend_len; m_start += BLOCK_M) {
          ...
          if (prefix_len > 0) {
            ...
            if (prefix_len > 0) {
              ...
              for (int ck = 0; ck < prefix_len; ck += vl_idx_max) {
                ...
                for (int j = 0; j < n_size; ++j) {
                  ...
                  if constexpr (std::is_same_v<scalar_t, at::Half>) {
                    for (int d = 0; d < head_size; d += vl_max) {
                      ...
                      vfloat16m2_t v_k = __riscv_vle16_v_f16m2(reinterpret_cast<const _Float16*>(k_src + d), vl);
          ...
          for (int n_start = 0; n_start < extend_len; n_start += BLOCK_N) {
            ...
            std::memset(v_buf, 0, BLOCK_N * head_size_v * sizeof(scalar_t));
