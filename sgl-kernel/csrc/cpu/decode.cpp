...
#include "vec.h"
  ...
  // Clang 19 version check
    ...
    #error "RVV backend in decode.cpp requires Clang compiler. Please use Clang 19.1.0 or later to compile this file."
  #endif
...
inline void prob_value_aggregate_rvv(
  ...
  for (int64_t d = 0; d < head_dim; d += vl) {
    ...
    vfloat32m1_t vaccum = __riscv_vfmv_v_f_f32m1(0.0f, current_vl);
    for (int64_t n = 0; n < N; ++n) {
      float prob = probs[n];
      ...
      vaccum = __riscv_vfmacc_vf_f32m1(vaccum, prob, vv, current_vl);
    }
    ...
    vout_part = __riscv_vfadd_vv_f32m1(vout_part, vaccum, current_vl);
