...
struct tinygemm_kernel_nn {
  ...
  static inline void apply(
    ...
    for (int64_t j = 0; j < n_size; j += vl_max) {
      ...
      // Unroll factor: BLOCK_M<=3 keeps 4 B-vectors + accumulators within 32 VRs (m4);
      // BLOCK_M==4 uses 2 to avoid register spill (4 acc + 4 B = 32 VRs, no margin).
      constexpr int64_t UNROLL = (BLOCK_M <= 3) ? 4 : 2;
      constexpr int64_t PREFETCH_DIST = UNROLL * 2;

      int64_t k = 0;
      // Main loop: process UNROLL k-steps per iteration to expose pipeline parallelism.
      // Note: RVV vector types are sizeless and cannot be stored in arrays.
      // Use named variables + if constexpr to handle UNROLL=2 and UNROLL=4 cases.
