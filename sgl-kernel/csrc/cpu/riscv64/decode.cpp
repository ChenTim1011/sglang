...
// Type trait to detect quantized types
template <typename T>
...
struct tinygemm_kernel_nt {
  static inline void apply(
      ...
      int64_t K,
      int64_t max_tokens,
      float k_scale = 1.0f) {  // Only used for INT8
    for (int64_t m = 0; m < BLOCK_M; ++m) {
      for (int64_t n = 0; n < BLOCK_N; ++n) {
        ...
        for (int64_t k = 0; k < K; ++k) {
          float a_val = load_and_dequant(A + m * lda + k, k_scale);
          float b_val = load_and_dequant(B + b_idx * ldb + k, k_scale);
...
struct tinygemm_kernel_nt<at::BFloat16, at::BFloat16, index_t, BLOCK_M, BLOCK_N> {
  static inline void apply(
      ...
      int64_t K,
      int64_t max_tokens,
      float k_scale = 1.0f) {  // Unused for BF16
    (void)k_scale;
...
struct tinygemm_kernel_nt<at::Half, at::Half, index_t, BLOCK_M, BLOCK_N> {
  static inline void apply(
      ...
      int64_t K,
      int64_t max_tokens,
      float k_scale = 1.0f) {  // Unused for FP16
    (void)k_scale;
    constexpr int ROWS = BLOCK_M;
    constexpr int COLS = BLOCK_N;
...
inline void tinygemm_kernel_nn_scalar(
    ...
    int64_t ldc,
    int64_t max_tokens,
    float v_scale = 1.0f) {  // Only used for INT8
  for (int64_t m = 0; m < M; ++m) {
    for (int64_t n = 0; n < N; ++n) {
      ...
      for (int64_t k = 0; k < K; ++k) {
        ...
        TORCH_CHECK(b_idx < max_tokens, "token index out of scope!");
        float b_val = load_and_dequant(B + b_idx * ldb + n, v_scale);
...
struct tinygemm_kernel_nn {
  static inline void apply(
    ...
    tinygemm_kernel_nn_scalar<scalar_t, kv_t, index_t>(
        A, B, C, indices, scale, BLOCK_M, BLOCK_N, K, lda, ldb, ldc, max_tokens, v_scale);
  }
};
...
struct tinygemm_kernel_nn<at::BFloat16, at::BFloat16, index_t, BLOCK_M, BLOCK_N> {
  static inline void apply(
      ...
      int64_t K,
      int64_t max_tokens,
      float v_scale = 1.0f) {  // Unused for BF16
    (void)v_scale;
    constexpr int ROWS = BLOCK_M;
    constexpr int COLS = 4;  // 4 * 8(vl) = 32 elements per tile
...
struct tinygemm_kernel_nn<at::Half, at::Half, index_t, BLOCK_M, BLOCK_N> {
  static inline void apply(
      ...
      int64_t K,
      int64_t max_tokens,
      float v_scale = 1.0f) {  // Unused for FP16
    (void)v_scale;
    constexpr int ROWS = BLOCK_M;
...
void index_gemm_kernel_nn(
  ...
  // N must be multiple of 16 for vectorized path
  if ((N & (kVecSize - 1)) != 0) {
    tinygemm_kernel_nn_scalar<scalar_t, kv_t, index_t>(
        A, B, C, indices, scale, M, N, K, lda, ldb, ldc, max_tokens, v_scale);
    return;
  }
...
void decode_set_kv_buffer_int8_quantize(
  ...
  at::parallel_for(0, batches * num_heads_kv, 0, [&](int64_t begin, int64_t end) {
    ...
    for (int64_t i = begin; i < end; i++) {
      ...
      const scalar_t* new_key_ptr = key + bs * nk_strideN + head_kv_id * nk_strideH;
      quantize_and_copy(k_buffer_ptr, new_key_ptr, head_size, k_scale);

      if (!is_mla) {
        int8_t* v_buffer_ptr = reinterpret_cast<int8_t*>(v_buffer + loc_val * v_strideN + head_kv_id * v_strideH);
        const scalar_t* new_value_ptr = value + bs * nv_strideN + head_kv_id * nv_strideH;
        quantize_and_copy(v_buffer_ptr, new_value_ptr, head_size_v, v_scale);
      }
...
template <typename scalar_t, typename kv_t, typename index_t, int64_t BLOCK_N_IGNORED>
  ...
  AT_DISPATCH_RVV_TYPES(query.scalar_type(), "decode_attention_int8_kernel", [&] {
    {
      AT_DISPATCH_INDEX_TYPES(index_dtype, "decode_attention_indices", [&] {
        ...
        if (num_heads == num_heads_kv) {
          ...
          constexpr int64_t BLOCK_N_IGNORED = 4;  // Not used by unified kernel
          decode_attention_kernel_impl<scalar_t, int8_t, index_t, BLOCK_N_IGNORED>(
              output.data_ptr<scalar_t>(),
              attn_logits.data_ptr<float>(),
              query.data_ptr<scalar_t>(),
