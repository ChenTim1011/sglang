...
class TestRVVBackendInitAndRegistration(unittest.TestCase, _RVVBackendTestMixin):
    ...
    def test_cpu_graph_registers_int8_attention_fake_ops(self):
        ...
        import sglang.srt.model_executor.cpu_graph_runner as cpu_graph_runner
        ...
        with (
            patch.object(cpu_graph_runner, "is_host_cpu_riscv", return_value=True),
            patch.object(
                cpu_graph_runner.torch,
                "ops",
                SimpleNamespace(
                    sgl_kernel=SimpleNamespace(
                        decode_attention_int8_cpu=SimpleNamespace(default=object()),
                        extend_attention_int8_cpu=SimpleNamespace(default=object()),
                    )
                ),
            ),
            patch("torch.library.register_fake", side_effect=fake_register_fake),
        ):
            cpu_graph_runner.register_fake_ops(tp_size=1)
