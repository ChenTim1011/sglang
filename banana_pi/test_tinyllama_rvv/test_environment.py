...
try:
    ...
    print(
        "  💡 Install Pillow: pip install Pillow --index-url https://gitlab.com/api/v4/projects/riseproject%2Fpython%2Fwheel_builder/packages/pypi/simple"
    )
...
def test_imports():
    ...
    try:
        ...
        if "libomp" in str(e).lower() or "undefined symbol" in str(e).lower():
            ...
            print(
                "     - libomp should be installed automatically by setup_banana_pi.sh"
            )
...
if __name__ == "__main__":
    ...
    if venv_ok and imports_ok and config_ok:
        ...
        if not venv_ok:
            ...
            print(
                "   Run: source ~/.local_riscv_env/workspace/venv_sglang/bin/activate"
            )
        if not libomp_configured:
            print("\n💡 libomp not found. To set up:")
            print(
                "   1. libomp should be installed automatically by setup_banana_pi.sh"
            )
